...
"스위칭 밴드를 ±10%p(80%/60%) 대신 ±20%p(90%/50%)로 넓히면 어떨까?"라는 의문에 대해 14년간의 실제 데이터(2012~2025년)로 검증한 결과다.

| 밴드 폭 기준 | 거치식 연평균(CAGR) | 거치식 2022년 MDD | 14년 거치식 스위칭 | 적립식 14년 최종 자산 | 적립식 강제 스위칭 |
| :--- | :---: | :---: | :---: | :---: | :---: |
| **±10%p (권장: 80% / 60%)** | **28.50%** | **-49.23%** | **8회** | **8억 1,923만 원** | **3회** |
| ±20%p (초게으름: 90% / 50%) | 28.84% | -54.01% | 1회 | 8억 185만 원 | **0회** |

* **±20%p의 특징 (완전 방치형):**
  * 매달 월급으로 부족한 쪽을 사주는 스마트 적립 덕분에, **14년 동안 주식을 서로 맞바꾸는 강제 스위칭이 단 0회(0번)** 발생했습니다. 주식을 파는 행위 자체를 극도로 싫어하는 투자자에게는 매력적입니다.
* **±10%p를 권장하는 이유 (황금 밸런스):**
  * ±20%p로 넓히면 레버리지가 50%까지 떨어져야만 스위칭이 발동하므로, 2022년 폭락장 당시 배당주로 레버리지 바닥을 줍는 타이밍이 늦어집니다.
  * 그 결과 **2022년 낙폭이 -49.2%에서 -54.0%로 약 5%p 더 깊어지며**, 반등장 최종 자산도 적립식 기준 약 1,738만 원 적었습니다.
