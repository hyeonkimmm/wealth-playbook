...
### 4) 리밸런싱 밴드 폭 비교 분석: ±10%p vs ±20%p
"스위칭 밴드를 ±10%p(80%/60%) 대신 ±20%p(90%/50%)로 넓히면 어떨까?"라는 의문에 대해 13년간의 실제 데이터로 검증한 결과다.

| 밴드 폭 기준 | 거치식 연평균(CAGR) | 거치식 2022년 MDD | 13년 거치식 스위칭 | 적립식 13년 최종 자산 | 적립식 강제 스위칭 |
| :--- | :---: | :---: | :---: | :---: | :---: |
| **±10%p (권장: 80% / 60%)** | **28.86%** | **-49.23%** | **7회** | **6억 5,984만 원** | **3회** |
| ±20%p (초게으름: 90% / 50%) | 29.02% | -54.01% | 1회 | 6억 3,503만 원 | **0회** |

* **±20%p의 특징 (완전 방치형):**
  * 매달 월급으로 부족한 쪽을 사주는 스마트 적립 덕분에, **13년 동안 주식을 서로 맞바꾸는 강제 스위칭이 단 0회(0번)** 발생했습니다. 주식을 파는 행위 자체를 극도로 싫어하는 투자자에게는 매력적입니다.
* **±10%p를 권장하는 이유 (황금 밸런스):**
  * ±20%p로 넓히면 레버리지가 50%까지 떨어져야만 스위칭이 발동하므로, 2022년 폭락장 당시 배당주로 레버리지 바닥을 줍는 타이밍이 늦어집니다.
  * 그 결과 **2022년 낙폭이 -49.2%에서 -54.0%로 약 5%p 더 깊어지며**, 반등장 수익도 적립식 기준 약 2,400만 원 적었습니다.
  * 따라서 **낙폭을 -40%대로 방어하고 하락장 바닥을 가장 적절하게 주워 담는 최적의 균형점(Sweet Spot)은 ±10%p**입니다.

---

## 5. 백테스트 실전 검증 (2012~2024년 13년간)

실제 미국 증시 데이터(배당 재투자 반영 수정주가)를 바탕으로, SCHD가 상장된 이후 13년간의 장기 성과를 직접 시뮬레이션한 결과다.

![13년간 백테스트 성과 비교](assets/backtest-cagr-chart.svg)

### 1) 일시불 거치식 투자 성과 (12년 보유)

| 전략 / 자산 | 총수익률 | 연평균 성장률(CAGR) | 전체 MDD | 2022년 수익률 | 2022년 MDD | 리밸런싱 횟수 |
| :--- | :---: | :---: | :---: | :---: | :---: | :---: |
| **★ 7:3 바벨 리밸런싱** | **+2,593.7%** | **28.86%** | **-49.48%** | **-46.67%** | **-49.23%** | **단 7회** |
| QLD (2배 레버리지 100%) | +4,159.6% | 33.48% | -63.68% | -61.33% | -63.08% | - |
| QQQ (나스닥 100 지수) | +918.4% | 19.56% | -35.12% | -33.22% | -34.83% | - |
| SPY (S&P 500 지수) | +483.8% | 14.55% | -33.72% | -18.65% | -24.50% | - |
| SCHD (배당다우존스 100%) | +364.6% | 12.55% | -33.37% | -3.23% | -16.84% | - |

### 2) 월 100만 원 적립식 투자 성과 (실제 직장인 시나리오)

매월 100만 원씩 13년간 꾸준히 적립하여 총 원금 1억 100만 원을 납입했을 때의 실제 최종 계좌 평가액 비교다.

| 전략 구분 | 총 납입 원금 | 최종 평가 금액 | 누적 수익률 | 적립식 MDD |
| :--- | :---: | :---: | :---: | :---: |
| **★ 7:3 바벨 적립** | **1억 100만 원** | **6억 3,503만 원** | **+528.7%** | **-51.70%** |
| QLD 적립 (레버리지 100%) | 1억 100만 원 | 9억 4,740만 원 | +838.0% | -62.84% |
| QQQ 적립 (나스닥 100) | 1억 100만 원 | 3억 8,295만 원 | +279.2% | -32.92% |
| SPY 적립 (S&P 500) | 1억 100만 원 | 2억 6,555만 원 | +162.9% | -33.72% |
| SCHD 적립 (배당 100%) | 1억 100만 원 | 2억 2,199만 원 | +119.8% | -33.37% |
...
1. **QQQ 대비 낙폭은 방어하고 수익은 압도적으로 극대화:**  
   ...
   * 결정적으로 폭락장 이후 반등 사이클(2023~2024년)을 거치며, 적립식 기준 최종 자산은 **QQQ 적립(3억 8,295만 원) 대비 7:3 바벨 적립(6억 3,503만 원)이 무려 2억 5,000만 원 이상(+66%p 초과) 더 크게 형성**되었습니다.
   * 즉, 3배 레버리지(TQQQ, -80% 이상 폭락)처럼 계좌가 영구 손상될 위험을 배제하면서도, QQQ의 낙폭에서 감내 가능한 수준의 변동성만 허용하는 대가로 1.7배 이상의 자산을 축적할 수 있습니다.

2. **2배 레버리지(QLD 100%) 몰빵의 현실적 한계 극복:**  
   * QLD 단독 투자는 수치상 연평균 수익률(33.5%)이 높아 보이지만, 2022년에 -63.7% 폭락을 견뎌야 합니다. 1억 원이 3,600만 원으로 쪼그라드는 공포 속에서 패닉 셀링(투매)을 하지 않고 버틸 수 있는 투자자는 극히 드뭅니다.
   * 7:3 바벨 전략은 매달 들어오는 월배당금과 상대적으로 덜 떨어진 SCHD를 매도해 바닥의 레버리지를 줍는 구조이므로 심리적 붕괴(뇌동매매)를 사전에 차단합니다.

3. **13년간 강제 리밸런싱은 단 7회:**  
   * 매일 호가창을 보고 긴장할 필요가 없습니다. 13년(3,269 거래일) 동안 ±10%p를 벗어나 강제로 스위칭을 실행한 횟수는 총 7회에 불과했습니다.
   * 평소에는 그저 월급날 부족한 쪽을 사기만 하면 되므로, 본업에 지장 없이 장기 투자를 우직하게 완주할 수 있습니다.
